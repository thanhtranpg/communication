<?php
class Admin_product_catForm extends Form{
	function Admin_product_catForm(){
		Form::Form('Admin_product_catForm');	
	...
	function on_submit(){
		...
		switch ($cmd){
			case 'addcat':
				self::add_product_cat();
				break;
			case 'editcat':
				self::edit_product_cat();
				break;
			case 'listcat':
				self::del_product_cat();
				break;
			default:
	...
	function add_product_cat(){
		...
		$new_catid = DB::insert(PREFIX_TABLE."product_cat",$arr);
       
        if($new_catid){			
			if($_FILES['image']['name']){
			
                 $img = ImageUrl::UploadItemImage($_FILES['image'],$new_catid,array(150,1680),'product_cat');
                 
				if(!is_int($img)){
				  
					DB::update(PREFIX_TABLE."product_cat",array('image'=>$img),"catid=".$new_catid);
				}else {
	...
	function edit_product_cat(){
      ...
      //  var_dump($_FILES);die;
        if($_FILES['image']['name']){
            ...
            $img = ImageUrl::UploadItemImage($_FILES['image'],$catid,array(150,1680),'product_cat');
			if(!is_int($img)){
				$arr['image'] = $img;
       ...
       $arr['url']= System::safe_title($arr['title']);
        //var_dump($arr);
		$new_catid = DB::update(PREFIX_TABLE."product_cat",$arr,"catid = $catid");
		if($new_catid){
			Url::redirect_url('webadmin.php?main='.CGlobal::$main.'&cmd=listcat');
		}else {
			$this->setFormError('tbao',"Không cập nhật được CSDL, mời bạn thử lại!");
		...
		if(count($ids)>0){
			for($i=0;$i<count($ids);$i++){
				if($ids[$i]){
					...
					DB::query("DELETE FROM ".PREFIX_TABLE."product WHERE catid = $id_del OR parentid = $id_del");
				}
			}			
			DB::query("DELETE FROM ".PREFIX_TABLE."product_cat WHERE catid IN(".$catids.")");
			Url::redirect_url('webadmin.php?main='.CGlobal::$main.'&cmd=listcat');
		}
	...
	//	$this->beginForm();	
		...
		switch ($cmd){
			...
			case 'editcat':
				$catid = System::getParamInt('catid');
				if($catid){									
					$item=DB::select(PREFIX_TABLE."product_cat", "catid = $catid");
					if(empty($item)){
			...
			case 'delcat':	
				...
				if($catid) {
					//Xoa tat ca cac tin thuoc catid nay
					DB::query("DELETE FROM ".PREFIX_TABLE."product WHERE catid = $catid OR parentid = $catid");
					DB::delete(PREFIX_TABLE.'product_cat',"catid = $catid");				
				}
				Url::redirect_url('webadmin.php?main='.CGlobal::$main.'&cmd=listcat');
				break;
			default:	
		...
		$display->add('cmd',$cmd);			
		$display->output('Admin_product_cat');					
	//	$this->endForm();
	}
