<?php
class Admin_advForm extends Form{
	function Admin_advForm(){
		Form::Form('Admin_advForm');	
	...
	function draw(){
		...
		switch ($cmd){
			...
			default:	
				...
				$adv_in='0';
				foreach (CGlobal::$adv_zone as $key => $zone) {
					$adv_in .= ','.$key;
				}

				$where = "Where catid in (".$adv_in.")";
				if(!empty($keyword)){
				...
				if($total){
					...
					$sql = "SELECT * FROM ".PREFIX_TABLE."adv $where ORDER BY catid, ord $limit";
