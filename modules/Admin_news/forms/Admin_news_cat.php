<?php
class Admin_news_catForm extends Form{
	function Admin_news_catForm(){
		Form::Form('Admin_news_catForm');	
	...
	function add_news_cat(){
		...
		$new_catid = DB::insert(PREFIX_TABLE."news_cat",$arr);
       
        if($new_catid){			
			if($_FILES['image']['name']){
			
                 $img = ImageUrl::UploadItemImage($_FILES['image'],$new_catid,array(150,1680),'news_cat');
                 
				if(!is_int($img)){
				  
					DB::update(PREFIX_TABLE."news_cat",array('image'=>$img),"catid=".$new_catid);
				}else {
	...
	function edit_news_cat(){
      ...
      //  var_dump($_FILES);die;
        if($_FILES['image']['name']){
            ...
            $img = ImageUrl::UploadItemImage($_FILES['image'],$catid,array(150,1680),'news_cat');
			if(!is_int($img)){
				$arr['image'] = $img;
       ...
       $arr['url']= System::safe_title($arr['title']);
        //var_dump($arr);
		$new_catid = DB::update(PREFIX_TABLE."news_cat",$arr,"catid = $catid");
		if($new_catid){
			Url::redirect_url('webadmin.php?main='.CGlobal::$main.'&cmd=listcat');
		}else {
			$this->setFormError('tbao',"Không cập nhật được CSDL, mời bạn thử lại!");
		...
		if(count($ids)>0){
			for($i=0;$i<count($ids);$i++){
				if($ids[$i]){
					...
					DB::query("DELETE FROM ".PREFIX_TABLE."news WHERE catid = $id_del OR parentid = $id_del");
				}
			}			
			DB::query("DELETE FROM ".PREFIX_TABLE."news_cat WHERE catid IN(".$catids.")");
			Url::redirect_url('webadmin.php?main='.CGlobal::$main.'&cmd=listcat');
		}
	...
	//	$this->beginForm();	
		...
		switch ($cmd){
			...
			case 'editcat':
				$catid = System::getParamInt('catid');
				if($catid){									
					$item=DB::select(PREFIX_TABLE."news_cat", "catid = $catid");
					if(empty($item)){
			...
			case 'delcat':	
				...
				if($catid) {
					//Xoa tat ca cac tin thuoc catid nay
					DB::query("DELETE FROM ".PREFIX_TABLE."news WHERE catid = $catid OR parentid = $catid");
					DB::delete(PREFIX_TABLE.'news_cat',"catid = $catid");				
				}
				Url::redirect_url('webadmin.php?main='.CGlobal::$main.'&cmd=listcat');
				break;
			default:	
