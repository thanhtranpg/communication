<?php
class Admin_phoneForm extends Form{
	function Admin_phoneForm(){
		Form::Form('Admin_phoneForm');	
	...
	function draw(){
		...
		switch ($cmd){
			...
			case 'edit':				
				$id = System::getParamInt('id');
				if($id){									
					$item=DB::select(PREFIX_TABLE."phone", "id = $id");
					if(empty($item)){
			...
			default:	
				...
				$s_catid = System::getParamInt('cat');
				...
				if($s_catid){
					$where .= (empty($where)) ? " WHERE " : " AND ";
					$where .= " cat = $s_catid OR cat = $s_catid ";
				}
				...
				$sql=" SELECT COUNT(*) AS total_row FROM ". PREFIX_TABLE ."phone $where";
				...
				if($total){
					...
					$sql = "SELECT * FROM ".PREFIX_TABLE."phone $where ORDER BY time DESC $limit";
